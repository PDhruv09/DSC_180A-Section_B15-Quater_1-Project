...
Implements a user-level (adds/removes-one-user) Laplace DP mechanism for the telemetry dataset, BY HAND.
...
5) Add Laplace noise with analytically chosen scale to U_total and U_error.
...
generate synthetic data or for direct evaluation (L_inf, IOU) in dp_eval_laplace.py.
"""
...
# L1 sensitivity of the user-level count vector:
...
# in L1 norm between neighboring datasets is 1 + 1 = 2.
L1_SENSITIVITY = 2.0
...
# -----------------------------
#  Laplace DP mechanism
# -----------------------------

def laplace_scale(l1_sensitivity: float, epsilon: float):
    """
    Compute the Laplace noise scale (b) for epsilon-DP
    using the standard Laplace mechanism:

        b = Δ_1 / ε

    where:
        Δ_1 = L1 sensitivity of the vector-valued function.
    """
...
def add_laplace_noise_to_counts(
    ...
    """
    Apply the Laplace mechanism to the user-level counts (U_total, U_error)
    ...
    L1 sensitivity is 1 + 1 = 2.

    We then add Laplace(0, b) noise independently to EACH coordinate,
    where b = L1_SENSITIVITY / epsilon.
    ...
    # Compute scale parameter from (epsilon, L1_sensitivity)
    scale = laplace_scale(L1_SENSITIVITY, epsilon)
    print(f"[dp_mechanism] Using scale = {scale:.4f} for Laplace noise.")

    # Add Laplace noise to U_total and U_error
    for col in ["U_total", "U_error"]:
        # Generate Laplace noise: Laplace(0, scale)
        # np.random.laplace uses scale parameter directly
        noise = rng.laplace(loc=0.0, scale=scale, size=len(noisy))
...
def main():
    ...
    - Apply Laplace DP to get NOISY counts / z-scores.
    ...
    print("[dp_mechanism] Applying Laplace DP to counts...")
    noisy_counts = add_laplace_noise_to_counts(
        true_counts, epsilon=DEFAULT_EPSILON, random_state=42
    )
    ...
    # (No extra merge needed; add_laplace_noise_to_counts worked on true_counts.)

    # Save for inspection
    out_path = os.path.join(REPORT_DIR, "dp_user_level_counts_and_zscores_laplace.csv")
